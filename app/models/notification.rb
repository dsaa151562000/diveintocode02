class Notification < ActiveRecord::Base
    ...
    #def selfクラスメソッドはインスタンスに依存しない共通的なメソッドを定義するために利用します。
    def self.sending_pusher(channel_user_id)
        Pusher.trigger("notifications#{channel_user_id}", 'message', {unread_count: Notification.unread_count(channel_user_id)})
    end
